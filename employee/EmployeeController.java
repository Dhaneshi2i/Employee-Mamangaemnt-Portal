...
public class EmployeeController {
	...
	DateTimeFormatter format = DateTimeFormatter.ofPattern("dd-MM-yyyy");
        ...
        try {
            boolean isCheckFirstName = true;
            while(isCheckFirstName) {
                logger.info("Enter your first name: ");
	        String firstName = scanner.next();
                boolean isValidFirstName = EmployeeUtil.isValidFirstName(firstName);
                if (isValidFirstName && userInput == 1) {
                    traineeDto.setFirstName(firstName);
                    isCheckFirstName = false;
                } else if (isValidFirstName && userInput == 2) {
                    trainerDto.setFirstName(firstName);
                    isCheckFirstName = false;
                } else {
                    logger.error("Enter Valid first name");
                }
            }
	        ...
	        String lastName = scanner.next();
                boolean isValidLastName = EmployeeUtil.isValidLastName(lastName);
                if (isValidLastName && userInput == 1) {
                    traineeDto.setLastName(lastName);
                    isCheckLastName = false;
                } else if (isValidLastName && userInput == 2) {
                    trainerDto.setLastName(lastName);
                    isCheckLastName = false;
                } else {
                    logger.error("Enter a valid last name");
                }
            }

            boolean isCheckContactNumber = true;
            while (isCheckContactNumber) {
                logger.info("Enter your Contact Number without country code: ");
                String contactNumber = scanner.next();
                boolean isValidateContactNumber = EmployeeUtil.isValidateContactNumber(contactNumber);
                if (isValidateContactNumber && userInput == 1) {
                    long validatedContactNumber = Long.parseLong(contactNumber);
                    traineeDto.setContactNumber(validatedContactNumber);
                    isCheckContactNumber = false;
                } else if (isValidateContactNumber && userInput == 2) {
                    long validatedContactNumber = Long.parseLong(contactNumber);
                    trainerDto.setContactNumber(validatedContactNumber);
                    isCheckContactNumber = false;
                } else {
                    logger.error(" Enter a valid MobileNumber");
                }
            }
